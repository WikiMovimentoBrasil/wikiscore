...
//Calcula número total de dias do wikiconcurso
...
SELECT
   date_generator.date as the_date,
   IFNULL(SUM(`queried`.`bytes`), 0) as count
from (
   select DATE_ADD(@date_min, INTERVAL (@i:=@i+1)-1 DAY) as `date`
   from information_schema.columns,(SELECT @i:=0) gen_sub 
   where DATE_ADD(@date_min,INTERVAL @i DAY) BETWEEN @date_min AND @date_max
    ...
    <body>
        ...
        <div class="w3-container">
            ...
            <div class="w3-row-padding"> 
                ...
                <div class="w3-third w3-section">
                    <div class="w3-card w3-light-grey w3-justify">
                        ...
                        <div class="w3-container">
                            <p class="w3-small">
                                ...
                                <br>
                                <?php echo(date('d/m/Y H:i:s (\U\T\C)', $contest['start_time'])); ?>
                            </p>
                            <p class="w3-small">
                                <b>Horário de término do wikiconcurso</b>
                                <br>
                                <?php echo(date('d/m/Y H:i:s (\U\T\C)', $contest['end_time'])); ?>
                            </p>
                            <p class="w3-small">
                                <b>Horário da última edição avaliada</b>
                                <br>
                                <?php echo(date('d/m/Y H:i:s (\U\T\C)', $lastedit)); ?>
                            </p>
                        </div>
        ...
        <script type="text/javascript">
            const dias = [ <?php for ($i=1; $i < $days; $i++) echo("'{$i}', "); ?> ];
                        ...
                        datasets: [
                            {
                                ...
                                data: [ <?php while ($row = mysqli_fetch_assoc($total_edits)) echo("{$row['count']}, "); ?> ],
                                fill: false,
                        ...
                        datasets: [
                            {
                                ...
                                data: [ <?php while ($row = mysqli_fetch_assoc($valid_edits)) echo("{$row['count']}, "); ?> ],
                                fill: false,
                        ...
                        datasets: [
                            {
                                ...
                                data: [ <?php while ($row = mysqli_fetch_assoc($new_articles)) echo("{$row['count']}, "); ?> ],
                                fill: false,
            ...
            const new_bytes = new Chart(
                document.getElementById('new_bytes'),
                {
                    ...
                    data: {
                        ...
                        datasets: [
                            {
                                label: 'Bytes adicionados por dia',
                                data: [ <?php while ($row = mysqli_fetch_assoc($new_bytes)) echo("{$row['count']}, "); ?> ],
                                fill: false,
            ...
            const valid_bytes = new Chart(
                document.getElementById('valid_bytes'),
                {
                    ...
                    data: {
                        ...
                        datasets: [
                            {
                                label: 'Bytes validados por dia',
                                data: [ <?php while ($row = mysqli_fetch_assoc($valid_bytes)) echo("{$row['count']}, "); ?> ],
                                fill: false,
